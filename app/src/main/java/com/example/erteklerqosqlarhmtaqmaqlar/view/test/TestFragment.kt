package com.example.erteklerqosqlarhmtaqmaqlar.view.test
...
import com.example.erteklerqosqlarhmtaqmaqlar.databinding.FragmentTestBinding
...
class TestFragment : Fragment(R.layout.fragment_test) {
    ...
    private lateinit var currentQuestion: Test
    private var score = 0
    ...
    private val selectedQuestions = mutableListOf<Test>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        try {
            ...
            (requireActivity() as? MainActivity)?.let { mainActivity ->
                ...
                amirAudio = AndroidAudioPlayer(this@TestFragment.requireContext())
        ...
        binding.btnNext.setOnClickListener {
            try {
                when (binding.btnNext.text) {
                    ...
                    getString(R.string.finish_text) -> findNavController().navigate(
    ...
    private fun initObservers() {
        mainViewModel.testsLiveData.observe(viewLifecycleOwner) { questions ->
            selectedQuestions.clear()
            selectedQuestions.addAll(questions.shuffled().take(5)) // Убедимся, что выбираем 5 вопросов
            currentQuestionId = 0 // Обнуляем индекс текущего вопроса
            loadQuestion()
        }

        mainViewModel.testByIdLiveData.observe(viewLifecycleOwner) { question ->
